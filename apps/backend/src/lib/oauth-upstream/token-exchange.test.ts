...
describe("isUpstreamUnauthorizedError", () => {
  ...
  // StreamableHTTPError's constructor (@modelcontextprotocol/sdk) never
  // embeds the numeric status in `.message` text — it's a separate `.code`
  // property. granola's "Session expired. Please sign in again." carries
  // `code: 401` with zero digits anywhere in the message; a text-only
  // regex over `.message` must not be the only path to a match.
  it("classifies a StreamableHTTPError-shaped code:401 with a digit-free message as an auth error", () => {
    const error = new Error("Session expired. Please sign in again.");
    Object.assign(error, { code: 401 });
    expect(isUpstreamUnauthorizedError(error)).toBe(true);
  });

  it("classifies a code:403 JSON-RPC session-token envelope as an auth error", () => {
    const error = new Error(
      '{"jsonrpc":"2.0","error":{"code":-32003,"message":"Authentication failed: Invalid or expired session token"},"id":null}',
    );
    Object.assign(error, { code: 403 });
    expect(isUpstreamUnauthorizedError(error)).toBe(true);
  });

  it("does NOT classify a bare code:403 with an unrelated message as an auth error", () => {
    const error = new Error("Forbidden");
    Object.assign(error, { code: 403 });
    expect(isUpstreamUnauthorizedError(error)).toBe(false);
  });
});
