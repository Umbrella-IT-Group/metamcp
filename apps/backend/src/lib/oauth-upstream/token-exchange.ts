...
export function isUpstreamUnauthorizedError(error: unknown): boolean {
  ...
  // StreamableHTTPError (and similar SDK transport errors) carry the real
  // HTTP status as a numeric `.code` property; the constructor never
  // embeds it as text in `.message` (`Streamable HTTP error: <body>`).
  // A text-only regex over `.message` therefore misses every upstream
  // that doesn't independently repeat the status code in its response
  // body, which is the common case. Check `.code` first.
  let code: unknown;
  ...
  if (code === 403 || /\b403\b/.test(message)) {
    ...
    // Non-standard JSON-RPC-shaped auth envelopes (e.g. Glean's
    // `{"error":{"code":-32003,"message":"...Invalid or expired session
    // token"}}`) don't match the OAuth envelope above but still describe
    // an expired/invalid token rather than a permanent permission denial.
    if (/session token|token (?:is )?(?:expired|invalid)|invalid or expired/i.test(message)) {
      return true;
    }
  }
